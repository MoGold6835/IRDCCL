...
def cclch_ecmh(cluster_list, old_sample, train_sample, eval_sample, image_model, text_model, lr, image_dir, transform,
    ...
    while i < len(cluster_list):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                for data in train_cluster_dataloader:
                    new_train_index1 = data["index"].numpy()
                    new_train_image1 = data["image"]
                    new_train_text1 = data["text"]
                    if cuda:
            ...
            for data in train_cluster_dataloader:
                ...
                loss, dloss, closs, hloss = object_function(sim_new, image_results, n1o_image_features_temp,
                                                            text_results, n1o_text_features_temp, new_train_label2,
            ...
            loss_str += "dloss:" + " {}".format(torch.mean(dloss)) + "  " + "\t"
            loss_str += "closs:" + " {}".format(torch.mean(closs)) + "  " + "\t"
            loss_str += "hloss:" + " {}".format(torch.mean(hloss)) + "  " + "\t"
