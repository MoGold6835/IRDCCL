...
from loss.DGCPN_loss2022 import loss_cal
...
def DGCPN_ccmh(cluster_list, old_sample, train_sample, eval_sample, image_model, text_model, lr, image_dir, transform,
    ...
    while i < len(cluster_list):
        ...
        for epoch in range(epochs):
            ...
            for data in train_cluster_dataloader:
                ...
                loss_i, _ = loss_cal(image_code, text_code.sign().detach(), sim, I)
                loss_t, _ = loss_cal(image_code.sign().detach(), text_code, sim, I)
                loss = loss_t + loss_i
