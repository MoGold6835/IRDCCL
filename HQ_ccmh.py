...
def HQ_ccmh(cluster_list, old_sample, train_sample, eval_sample, image_model, text_model, lr, image_dir, transform,
    ...
    while i < len(cluster_list):
        ...
        for epoch in range(epochs):
            ...
            for data in train_cluster_dataloader:
                ...
                q_loss = quantization_loss(C, image_features1, image_code, text_features1, text_code)
                h_losses = h_loss(image_features1, text_features1)
                s_loss = similarity_loss(image_features1, text_features1, sim)
                loss = (q_loss + s_loss + h_losses[0] + h_losses[1]) / (batch_size * len(train_sample))
